...
public class CarGenerationController : MonoBehaviour
{
    ...
    private void InitialisePopulation()
    {
        for (int i = 0; i < populationSize; i++)
        {
            ...
            GameObject creature = Instantiate(creaturePrefab, pos, rot);
            CarAgent agent = creature.GetComponent<CarAgent>();
    ...
    private void BreedPopulation()
    {
        ...
        List<GameObject> sortedList = population.OrderByDescending(o => o.GetComponent<CarAgent>().survivalTime).ToList();

 
        if (sortedList[0].GetComponent<CarAgent>().survivalTime > maxSurvivalTime)
        {
            maxSurvivalTime = sortedList[0].GetComponent<CarAgent>().survivalTime;
        }
    ...
    private GameObject Breed(GameObject parent1, GameObject parent2)
    {
        ...
        GameObject offspring = Instantiate(creaturePrefab, pos, rot);
        CarAgent offspringAgent = offspring.GetComponent<CarAgent>();
        offspringAgent.controller = this;

        // Get the parents DNA
        CarAgent agent1 = parent1.GetComponent<CarAgent>();
        CarAgent agent2 = parent2.GetComponent<CarAgent>();
        ...
        offspring.GetComponent<CarAgent>().net = childNet;
